...
export function Textarea({ component, value, onChange, theme, disabled }: ComponentProps) {
  const primaryColor = theme?.primaryColor || '#3b82f6';
  const backgroundColor = theme?.backgroundColor || '#13111C';
  const textColor = theme?.textColor || '#ffffff';
  const mutedTextColor = textColor + '60';
  
  const baseClasses = `w-full px-4 py-3 border rounded-lg focus:outline-none transition-all duration-200 resize-y ${
    ...
    <div className="space-y-1">
      ...
      {component.max_length && (
        <p 
          ...
          style={{ color: mutedTextColor }}
        >
          {(value || '').length} / {component.max_length}
        </p>
      )}
    </div>
  );
